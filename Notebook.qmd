...
This notebook presents simulations of sublimating exocomets in the Beta Pictoris system. These simulations were performed using the **EVaporating Exoplanet** code (EVE), initially developed to model atmospheric escape from exoplanets (see, e.g., [Bourrier et al. 2014, A&A, 565](https://www.aanda.org/articles/aa/full_html/2014/05/aa23064-13), A105; [Bourrier et al. 2016, A&A, 591, A121](https://www.aanda.org/articles/aa/full_html/2016/07/aa28362-16))

The present notebook is available at https://github.com/TVrignaud/Exocomet_simulations
...
$\beta$ Pic was observed over more than 200 individual nights by the **HARPS spectrograph**, mounted on the 3.6m telescope of La Silla observatory, Chile. During most of these nights, the star was observed during several hours continuously, allowing the detection of numerous exocomet signatures in Ca II lines and a detailed monitoring of their **temporal evolution**. 
...
During both nights, the Ca II H&K lines displayed strong signatures from transiting exocomets, including signatures at low radial velocities (typically < 30 km/s) which do not vary substantially during the course of each night, and redshifted signatures at larger velocities (typically ~ 100 km/s) which show significant temporal variations. As shown by the absorption maps below, the velocities of these signatures tend to increase over time. As shown by [Kennedy, MNRAS, 479, 2](https://academic.oup.com/mnras/article/479/2/1997/5033700), these acceleration are naturally interpreted as reflecting the stellar gravity force, and therefore directly probe the transit distance of the objects. 

The goal of the simulations presented in the following will be to reproduce such high-velocity signatures
...
```{python}
# A function to overplot a list of spectra obtained durin a given night
...
def quick_plot(spec_dic, inst, date, line = None, reference = None, xlim = None, ylim = None, loc_label = 'lower right', title = None) : 
    ...
    for spec in spec_dic['spec_renorm'][inst][date] : 
        if spec == 'Average night' : width, alpha = 2.5, 1
        else                       : width, alpha = 1.2, 0.3
    ...
    fig.update_layout(
        ...
        text=title,
        x=0.5,        
        y= 1 - mt / total_height + 20/total_height,
        xanchor='center',
        yanchor='bottom'
       ),
...
def absorption_map(spec_dic, inst, date, line, reference, rv_range, flux_range, time_range = None, timestep = None, rvstep = None, title = None): 
    ...
    # Figure layout
    mt, mb = 65, 80  # margins
    total_height = 600 * (np.max(Y) - np.min(Y))/8 + mt + mb
    ml, mr = 100, 150
    total_width = 1000 + ml + mr
...
In addition to strong exocomet features at low velocities, a short exocomet event is observed between 4 and 7 hours after the beginning of the night, around $v = 80$ km/s. This event exhibit a strong acceleration (~ 12 km/s/h), placing the cometary cloud at about $\sim 8 R_\star$ from the star.
...
quick_plot(spec_dic, inst, date, line = 3933.663, reference = reference, xlim = rv_range, ylim = None, loc_label = 'lower right', title = date + r' - Ca II K - 3933.7 Å')
quick_plot(spec_dic, inst, date, line = 3968.469, reference = reference, xlim = rv_range, ylim = None, loc_label = 'lower right', title = date + r' - Ca II H - 3968.5 Å')
...
The situation here is similar to the one observed on December 5, 2017. Strong stable features are observed from -50 to +50 km/s, while several variable features can be observed around +100 km/s. The acceleration of these features is about 15-20 km/s, placing the comets at only $6-7 r_\star$ from $\beta$ Pic.
...
quick_plot(spec_dic, inst, date, line = 3933.663, reference = reference, xlim = rv_range, ylim = None, loc_label = 'lower right', title = date + r' - Ca II K - 3933.7 Å')
quick_plot(spec_dic, inst, date, line = 3968.469, reference = reference, xlim = rv_range, ylim = None, loc_label = 'lower right', title = date + r' - Ca II K - 3968.5 Å')
...
Here, I present a example of exocomet simulation, aimed at reproducing the high-velocity features observed on December 5, 2017 and March 30, 2018. These simulations were performed using the **EVaporating Exoplanet** code (EVE). This simulation incorporates several key physical ingredients : 

- We start by considering a cometary nucleus, orbiting $\beta$ Pic on a nearly parabolic orbit. Here, the periastron distance of the nucleus is 0.04 au (about $5.6 R_\star$) and the argument of periastron is 140°. The comet thus transits the star before its periastron. 
- The comet nucleus is assumed to only release dust particles. The production rate is taken to be proportionnal to $d^{-2}$, and is equal to $5\times10^5$ kg/s at 1 au from the star. The dust emission is isotropic, and dust grains are ejected with an escape velocity of 1 km/s.
...
- By absorbing the stellar photons, dust grains are repelled away from the star (radiation pressure) and are heated up. When hot enough (typically above 1200 K), the grains can sublimate, and their mass is entirely converted into gas. 
- The gas particles released from dust sublimation are assumed to have a typical cometary composition, as measured in [Vrignaud et al. 2025](https://www.aanda.org/articles/aa/full_html/2025/05/aa53568-24) (A&A, 697, A21) and [Vrignaud & Lecavelier 2026](https://www.researchsquare.com/article/rs-9515646/v1) (submitted to Nature Astronomy). Metals (Fe, Mg, Si...) and Oxygen are taken to be solar; C/Fe = 20; H = 2$\times$O; and noble gas are not considered. 
- The gas dynamics is computed by combining gravity and radiation pressure, and assuming a radiation pressure-to-gravity ratio ($\beta$) of 0.1. This reproduces the average $\beta$ factor of a gaseous tail with the elemental composition listed above and a fairly high level of ionisation ($\text{Fe}^+/\text{Fe} \sim 0.05$, $\text{Ca}^+/\text{Ca} \sim 0.005$; the rest being stored in $\text{Fe}^{2+}$ and $\text{Ca}^{2+}$). In this case, the radiation pressure is mainly dominated by Fe+ ions. 
- Finally, the absorption signature of the comet is computed in Fe II, Mg II and Ca II lines. Importantly, the comet's signatures in those lines are ponderated by the elemental abundances of Fe, Mg and Ca, by the populations of these elements in their singly ionised states, and, for Fe II, by the excitation state, assuming an excitation temperature of 8000 K. 


The evolution of the dust and gaseous tails of the comets are displayed in the video below, along with the comet's signatures in the two Fe II lines near 2600 A (rising from the ground and first excited state), in the Mg II k line at 2796 A, and in the Ca II K line at 3933 A. 

The simulation starts when the comet is far from the star ($\sim0.4$au). At such distance, dust sublimation is not very efficient, allowing a dust tail to form. Then, when the comet gets close to the star (within $\sim 0.1$ au), the dust grains are quickly sublimated, and are transformed into gas. When the gaseous tail transit the star, it produces absorption signature in the stellar spectrum, here computed in a few Fe II, Mg II and Ca II lines. Finally, after perihelion, the comet leaves the stellar vicinity, dust grains are not sublimated anymore, and the dust tail reforms.

***

```{python}
Video("Full_simu.mp4", width=1800, html_attributes="controls autoplay loop")
```

# Simulated absortion map in the Ca II K line


The comet displays a strong signature in Ca II lines from -1h to +3 relative to the nucleus mid-transit. The object acceleration is clearly visible; it reproduces well the signatures observed in the HARPS data.

```{python}
# Retrieve the simulated absorption in Fe II, Mg II and Ca II lines

spec_dic_simu = np.load('Data_comet_simu.npy', allow_pickle = True).item()
```

```{python}
# A function to plot the simulated comet signature around a given line over its full transit
#     + The function builds a map showing all spectra obtained during the simulation, similarly to 'absorption_map'
#     + 'wl_domain' can be 'Fe II', 'Mg II', 'Ca II K' or 'Ca II H'
#     + the reference line is specified through 'line'
...
```{python}
wl_domain = 'Ca II K'     
line = 3934.78              # Angstrom
time_range = [-2,4.5]     # hours
